...
HAND_OVERLAY_PATH = IMG_DIR / "ct_rucyfina_hand.png"
BASE_MAX_WIDTH = 640
BASE_MAX_HEIGHT = 480
...
def i_squeeze(images: list[BuildImage], texts, args):
    image = images[1]
    ...
    hand_overlay = BuildImage.open(HAND_OVERLAY_PATH).image.convert("RGBA")
    bg_w, bg_h = hand_overlay.size
    ...
    def maker(i: int) -> Maker:
        def build_base_canvas(background: Image.Image) -> Image.Image:
            background = ImageOps.exif_transpose(background).convert("RGBA")
            base_w, base_h = contain_size(background.size, (BASE_MAX_WIDTH, BASE_MAX_HEIGHT))
            resized_bg = background.resize((base_w, base_h), Image.Resampling.LANCZOS)
            canvas = Image.new("RGBA", (BASE_MAX_WIDTH, BASE_MAX_HEIGHT), (0, 0, 0, 0))
            canvas.paste(
                resized_bg,
                ((BASE_MAX_WIDTH - base_w) // 2, (BASE_MAX_HEIGHT - base_h) // 2),
            )
            canvas.paste(hand_overlay, (0, 0), hand_overlay)
            return canvas

        def compose_frame(base_canvas: Image.Image, foreground: Image.Image) -> BuildImage:
            canvas = base_canvas.copy()
            canvas.paste(foreground, (paste_x, paste_y), foreground)
            return BuildImage(canvas)

        def make(imgs: list[BuildImage]) -> BuildImage:
            base_canvas = build_base_canvas(imgs[0].image)
            img = imgs[1].image.convert("RGBA").resize((width, height))
            img.putalpha(ellipse_mask)
            if i <= 9:
                return compose_frame(base_canvas, img)
            elif 9 < i < 28:
                new_img = img.copy()
                ...
                return compose_frame(base_canvas, new_img)
            else:
                return compose_frame(base_canvas, make_dust(dusts, step, img.size))
